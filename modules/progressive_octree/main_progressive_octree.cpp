...
Uniforms getUniforms(shared_ptr<GLRenderer> renderer){
	...
	glm::mat4 world = renderer->camera->world;
	//glm::mat4 world;
...
void renderCUDA(shared_ptr<GLRenderer> renderer , bool* deviceDebug, MyData* ndcData, MyData* pointFloatData, MyMat4* matData){
	...
	for(string path : paths){
		...
		if(iEndsWith(path, "las") || iEndsWith(path, "laz")){
			...
			for (uint64_t first = 0; first < header.numPoints; first += MAX_BATCH_SIZE) {
				printfmt("adding batch to load queue: {} \n", path);
				printfmt("    first: {} \n", first);
				PointBatch batch;
				batch.file = path;
...
int main(){
	...
	// Define the number of elements you want to transfer
	const int numElements = 10;

	// Create an array of MyData to hold the data
	MyData hostndc[numElements];
	MyData hostpointfloat[numElements];
	MyMat4 hostmat[numElements];

	MyData* deviceNdc;
	cudaMalloc((void**)&deviceNdc, sizeof(MyData) * numElements); // numElements should be adjusted to your actual data size

	MyData* devicePointFloat;
	cudaMalloc((void**)&devicePointFloat, sizeof(MyData) * numElements); // numElements should be adjusted to your actual data size

	MyMat4* deviceMat;
	cudaMalloc((void**)&deviceMat, sizeof(MyMat4) * numElements); // numElements should be adjusted to your actual data size

	// allocate cuda mememory for storing debug
	cudaMalloc((void**)&deviceDebug, sizeof(bool));
	// set device debug to debug
	cudaMemcpy(deviceDebug, &debug, sizeof(bool), cudaMemcpyHostToDevice);
	...
	auto render = [&](){
		...
		renderCUDA(renderer, deviceDebug, deviceNdc, devicePointFloat, deviceMat);

		// flip debug again when its true
		// retrieve new debug value
		cudaMemcpy(&debug, deviceDebug, sizeof(bool), cudaMemcpyDeviceToHost);
		// Ensure all previous CUDA operations are completed and data is synchronized
		cudaDeviceSynchronize();
		if (debug==true){
			debug = !debug;
			cudaMemcpy(deviceDebug, &debug, sizeof(bool), cudaMemcpyHostToDevice);
			// print the value of debug as "the value of the debug variable is {} again"
			// printfmt("the value of the debug variable is {} again \n", debug);
		...
		{ // RENDER IMGUI SETTINGS WINDOW
			...
			if (ImGui::Button("Save Image and text")) {
    			std::cout << "Save image to disk" << std::endl;
    			...
    			do {
        			...
        			imgOss << baseFilename << "_" << std::setfill('0') << std::setw(2) << counter << ".png";
        			imgFilename = imgOss.str();

        			std::ostringstream txtOss;
        			txtOss << baseFilename << "_" << std::setfill('0') << std::setw(2) << counter << ".txt";
        			txtFilename = txtOss.str();

        			counter++;
    			} while (std::filesystem::exists(imgFilename)); //|| std::filesystem::exists(txtFilename));

    			captureAndSaveImage(renderer->width, renderer->height, imgFilename);

    			// Now, create and save the text file with camera data
    			auto controls = renderer->controls;
    			auto pos = controls->getPosition();
    			auto target = controls->target;
    			...
    			// Save the camera data to the text file
    			std::ofstream txtFile(txtFilename);
    			if (txtFile.is_open()) {
        			txtFile << str;
        			txtFile.close();
        			std::cout << "Camera data saved to " << txtFilename << std::endl;
    			}
			}
			 ...
			 if(ImGui::Button("print the matrices")){
				...
				cudaMemcpy(hostndc, deviceNdc, sizeof(MyData) * numElements, cudaMemcpyDeviceToHost);
				cudaMemcpy(hostpointfloat, devicePointFloat, sizeof(MyData) * numElements, cudaMemcpyDeviceToHost);
				cudaMemcpy(hostmat, deviceMat, sizeof(MyMat4) * numElements, cudaMemcpyDeviceToHost);

				// print the matrices
    			// Print the matrices and vectors
    			for (int i = 0; i < numElements; i++) {
        			printf("NDC %d: (%f, %f, %f, %f)\n", i, hostndc[i].x, hostndc[i].y, hostndc[i].z, hostndc[i].w);
        			printf("PointFloat %d: (%f, %f, %f, %f)\n", i, hostpointfloat[i].x, hostpointfloat[i].y, hostpointfloat[i].z, hostpointfloat[i].w);
        			printf("Matrix %d:\n", i);
        			for (int row = 0; row < 4; row++) {
            			printf("   (%f, %f, %f, %f)\n", hostmat[i].rows[row].x, hostmat[i].rows[row].y, hostmat[i].rows[row].z, hostmat[i].rows[row].w);
        			}
    			}
			 }

			if (ImGui::Button("Save Image and text(w. matrix)")) {
				debug = !debug;
				cudaMemcpy(deviceDebug, &debug, sizeof(bool), cudaMemcpyHostToDevice);
				cudaDeviceSynchronize();
				printfmt("debug = {} \n", debug ? "true" : "false");
    			...
    			// Now, create and save the text file with camera data and matrices
    			auto controls = renderer->controls;
    			auto pos = controls->getPosition();
    			auto target = controls->target;
    			...
    			// Append the matrices and vectors data
    			for (int i = 0; i < numElements; i++) {
					ss << "NDC " << i << ": ("
   					<< std::fixed << std::setprecision(2) << hostndc[i].x << ", "
   					<< hostndc[i].y << ", "
   					<< hostndc[i].z << ", "
   					<< hostndc[i].w << ")\n";

					ss << "PointFloat " << i << ": ("
   					<< std::fixed << std::setprecision(2) << hostpointfloat[i].x << ", "
   					<< hostpointfloat[i].y << ", "
   					<< hostpointfloat[i].z << ", "
   					<< hostpointfloat[i].w << ")\n";
					...
					for (int row = 0; row < 4; row++) {
    					ss << "   ("
       					<< std::fixed << std::setprecision(2) << hostmat[i].rows[row].x << ", "
       					<< hostmat[i].rows[row].y << ", "
       					<< hostmat[i].rows[row].z << ", "
       					<< hostmat[i].rows[row].w << ")\n";
					}
				...
				// Append glm matrices (world, view, proj)
				ss << "World Matrix:\n";
				for (int row = 0; row < 4; row++) {
    				ss << "   ("
       				<< std::fixed << std::setprecision(2) << world[row][0] << ", "
       				<< world[row][1] << ", "
       				<< world[row][2] << ", "
       				<< world[row][3] << ")\n";
				}
				...
				for (int row = 0; row < 4; row++) {
    				ss << "   ("
       				<< std::fixed << std::setprecision(2) << view[row][0] << ", "
       				<< view[row][1] << ", "
       				<< view[row][2] << ", "
       				<< view[row][3] << ")\n";
				}
				...
				for (int row = 0; row < 4; row++) {
    				ss << "   ("
       				<< std::fixed << std::setprecision(2) << proj[row][0] << ", "
       				<< proj[row][1] << ", "
       				<< proj[row][2] << ", "
       				<< proj[row][3] << ")\n";
				}
				
				double aspect = renderer->camera->aspect;
				double fovy = renderer->camera->fovy;
				double near = renderer->camera->near;
				double far = renderer->camera->far;
				int width = renderer->width;
				int height = renderer->height;


				// Append camera properties
				ss << "Aspect Ratio: " << std::fixed << std::setprecision(2) << aspect << "\n";
				ss << "Field of View (FOV): " << std::fixed << std::setprecision(2) << fovy << "\n";
				ss << "Near Clip Plane: " << std::fixed << std::setprecision(2) << near << "\n";
				ss << "Far Clip Plane: " << std::fixed << std::setprecision(2) << far << "\n";
				ss << "Width: " << std::fixed << std::setprecision(2) << width << "\n";
				ss << "Height: " <<std::fixed <<std::setprecision(2) <<height << "\n";

				double radius = renderer->controls->radius;
				double yaw = renderer->controls->yaw;
				double pitch = renderer->controls->pitch;
				//glm::dvec3 target = renderer->controls->target;

				glm::dvec3 up    = {0, 0, 1};
				glm::dvec3 right = {1, 0, 0};

				glm::mat4 translateRadius = glm::translate(
					glm::dmat4(), 
					glm::dvec3(0.0, 0.0, radius));
				glm::mat4 translateTarget = glm::translate(glm::dmat4(), target);
				glm::mat4 rotYaw = glm::rotate(yaw, up);
				glm::mat4 rotPitch = glm::rotate(pitch, right);


				ss << "Translate Radius Matrix:\n";
				for (int row = 0; row < 4; row++) {
    				ss << "   ("
        				<< std::fixed << std::setprecision(2) << translateRadius[row][0] << ", "
        				<< translateRadius[row][1] << ", "
        				<< translateRadius[row][2] << ", "
        				<< translateRadius[row][3] << ")\n";
				}

				ss << "Translate Target Matrix:\n";
				for (int row = 0; row < 4; row++) {
    				ss << "   ("
        				<< std::fixed << std::setprecision(2) << translateTarget[row][0] << ", "
        				<< translateTarget[row][1] << ", "
        				<< translateTarget[row][2] << ", "
        				<< translateTarget[row][3] << ")\n";
				}

				ss << "Rotate Yaw Matrix:\n";
				for (int row = 0; row < 4; row++) {
    				ss << "   ("
        				<< std::fixed << std::setprecision(2) << rotYaw[row][0] << ", "
        				<< rotYaw[row][1] << ", "
        				<< rotYaw[row][2] << ", "
        				<< rotYaw[row][3] << ")\n";
				}

				ss << "Rotate Pitch Matrix:\n";
				for (int row = 0; row < 4; row++) {
    				ss << "   ("
        				<< std::fixed << std::setprecision(2) << rotPitch[row][0] << ", "
        				<< rotPitch[row][1] << ", "
        				<< rotPitch[row][2] << ", "
        				<< rotPitch[row][3] << ")\n";
				}
    			...
    			// Save the camera data and matrices to the text file
    			std::ofstream txtFile(txtFilename);
    			if (txtFile.is_open()) {
        			txtFile << str;
        			txtFile.close();
        			std::cout << "Camera data and matrices saved to " << txtFilename << std::endl;
    			}
