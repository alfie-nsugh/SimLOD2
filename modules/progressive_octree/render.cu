...
void drawNode(Chunk* chunk, Node* node, uint32_t numElements, uint64_t* framebuffer, bool* deviceDebug, MyData* ndcData, MyData* pointFloatData, MyMat4* transformData){
	...
	for(
		...
		drawPoint(point, node, framebuffer, deviceDebug, ndcData, pointFloatData, transformData);
...
void drawNodes(Node* nodes, uint32_t numNodes, uint64_t* framebuffer, bool* deviceDebug, MyData* ndcData, MyData* pointFloatData, MyMat4* transformData){
	...
	while(true){
		...
		drawNode(node->points, node, node->numPoints, framebuffer, deviceDebug, ndcData, pointFloatData, transformData);
		drawNode(node->voxelChunks, node, node->numVoxels, framebuffer, deviceDebug, ndcData, pointFloatData, transformData);
...
void kernel_render(
	...
	{ // draw nodes, boxes, lines, ...
		if(uniforms.useHighQualityShading){
			...
			drawNodes(visibleNodes, numVisibleNodes, framebuffer, deviceDebug, ndcData, pointFloatData, matData);
		}
